      ...
      <div className="overflow-x-auto rounded-xl border-2 border-(--color-border) bg-(--color-surface) shadow-[6px_6px_0_rgba(139,94,60,0.25)] p-6">
        <div className="space-y-4 animate-pulse">
          ...
          {skeletonRows.map((_, index) => (
            <div key={index} className="flex items-center justify-between py-4 border-b border-dashed border-(--color-border)/40">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded bg-(--color-border)/30"></div>
                <div className="space-y-2">
                  <div className="h-3 w-32 rounded bg-(--color-border)/40"></div>
                  <div className="h-2 w-40 rounded bg-(--color-border)/20"></div>
                </div>
              </div>
              <div className="h-3 w-28 rounded bg-(--color-border)/30"></div>
              <div className="h-6 w-12 rounded bg-(--color-border)/20"></div>
            </div>
          ))}
        </div>
      </div>
