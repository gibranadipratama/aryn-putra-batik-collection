...
export default function DashboardClient({ stats }: { stats: any }) {
  const router = useRouter(); // <-- Tambahkan router untuk navigasi tabel
    ...
    <div className="mx-auto max-w-7xl font-sans text-(--color-text-primary)">
      ...
      <div className="mb-8 grid grid-cols-1 gap-6 sm:grid-cols-3">
        ...
        <Link 
          ...
          className="group block rounded-xl border-2 border-(--color-border) bg-(--color-surface) p-6 shadow-[4px_4px_0_rgba(139,94,60,0.15)] transition-all hover:-translate-y-1 hover:shadow-[6px_6px_0_rgba(139,94,60,0.25)]"
        >
          <div className="mb-4 flex items-center justify-between">
            <h3 className="text-[10px] font-bold uppercase tracking-widest text-(--color-text-secondary) transition group-hover:text-(--color-primary-dark)">
              Pendapatan Bulan Ini
            </h3>
            <div className="flex h-10 w-10 items-center justify-center rounded-md border-2 border-(--color-border) bg-(--color-primary-dark) text-(--color-accent) shadow-[2px_2px_0_rgba(58,40,27,0.4)] transition group-hover:scale-110">
              <DollarSign className="h-5 w-5" />
            </div>
          </div>
          
          <p className="text-2xl font-black text-(--color-primary-dark)">
            {formatRupiah(stats.thisMonthRevenue)}
          </p>
          
          <div className="mt-2 flex items-center justify-between">
            <p className="text-[9px] font-bold uppercase tracking-wider text-(--color-text-secondary)">
              Total: <span className="text-(--color-primary-dark)">{formatRupiah(stats.totalRevenue)}</span>
            </p>
            <span className="text-[9px] font-bold uppercase tracking-widest text-(--color-primary) opacity-0 transition group-hover:opacity-100">
              Detail &rarr;
            </span>
          </div>
        </Link>
              ...
              <table className="w-full text-left text-xs text-(--color-text-primary)">
                ...
                <tbody className="divide-y divide-dashed divide-(--color-border)">
                  {stats.recentOrders.map((order: any) => (
                    <tr 
                      key={order.id} 
                      onClick={() => router.push('/dashboard/pesanan')} // <-- Membuat baris bisa diklik
                      className="cursor-pointer transition hover:bg-(--color-border)/20"
                    >
                      <td className="px-6 py-4 font-black uppercase text-(--color-primary-dark)">{order.orderNumber || order.id.slice(0,8)}</td>
          ...
          <div className="flex flex-1 flex-col gap-4 p-6">
            {stats.lowStockItems.length === 0 ? (
              ...
              stats.lowStockItems.map((item: any) => (
                <Link 
                  href="/dashboard/produk" // <-- Membungkus item dengan Link agar bisa diklik
                  key={item.id} 
                  className="group flex items-center justify-between rounded-md border-2 border-(--color-danger)/30 bg-(--color-bg) p-3 transition hover:border-(--color-danger) hover:bg-(--color-danger)/10"
                >
                  <div>
                    <p className="line-clamp-1 text-xs font-bold uppercase text-(--color-primary-dark) transition group-hover:text-(--color-danger)">
                  ...
                  <div className="rounded border border-(--color-danger) bg-(--color-danger) px-3 py-1 text-center text-white shadow-[2px_2px_0_rgba(168,69,47,0.3)] transition group-hover:scale-105">
